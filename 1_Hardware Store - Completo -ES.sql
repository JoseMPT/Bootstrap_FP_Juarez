...
`user_password` VARCHAR(500) NOT NULL COMMENT 'Contraseña de usuario (obligatoria).',
...
`ticket_id` INT UNSIGNED PRIMARY KEY AUTO_INCREMENT NOT NULL COMMENT 'ID de ticket (valor único).')
...
CREATE PROCEDURE insert_sale_data (IN saleID VARCHAR(10), IN saleAmount INT, IN saleBuy INT) NOT DETERMINISTIC
    BEGIN
        ...
        IF saleID IN (SELECT sale_productId FROM sale_data)
               AND 0 = (SELECT sale_bought FROM sale_data WHERE saleID = sale_productId AND sale_bought = FALSE AND sale_buy = saleBuy)
            THEN
            UPDATE sale_data SET sale_amount = saleAmount, sale_total = total WHERE sale_productId = saleID AND sale_bought = FALSE AND sale_buy = saleBuy;
                ELSE
...
DELIMITER **
CREATE PROCEDURE getCartProducts (IN email VARCHAR(50)) NOT DETERMINISTIC
	BEGIN
	    SELECT sale_productId, product_name, product_description, sale_total, sale_amount FROM buy_products INNER JOIN sale_data INNER JOIN products_data
	    ON buy_id = sale_buy AND buy_user = email AND sale_bought = 0 AND product_id = sale_data.sale_productId;
	END**
